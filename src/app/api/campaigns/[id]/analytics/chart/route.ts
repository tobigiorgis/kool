  ...
  if (period === "1d") {
    ...
    for (let h = 0; h < 24; h++) {
      ...
      chartData.push({
        ...
        sales: conversions.filter((c) => c.convertedAt >= hourStart && c.convertedAt < hourEnd).length,
      })
    }
  } else {
    for (let d = 0; d < days; d++) {
      ...
      chartData.push({
        ...
        sales: conversions.filter((c) => c.convertedAt >= dayStart && c.convertedAt < dayEnd).length,
      })
    }
  }
