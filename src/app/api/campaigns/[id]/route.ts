...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [clickCount, conversions, commissions, linkClickCounts, linkConversionData] =
      await Promise.all([
        ...
        creatorIds.length
          ? prisma.conversion.findMany({
              where: {
                ...
                linkId: { in: linkIds.length ? linkIds : undefined },
              },
    ...
    const clicksByLink = new Map(
      (linkClickCounts as { linkId: string; _count: { id: number } }[]).map((c) => [
        c.linkId,
        c._count.id,
      ])
    )
    ...
    for (const conv of linkConversionData as { linkId: string | null; orderAmount: number }[]) {
      ...
      salesByLink.set(conv.linkId, {
        sales: prev.sales + 1,
        revenue: prev.revenue + conv.orderAmount,
      })
    }
    ...
    const totalRevenue = (conversions as { orderAmount: number }[]).reduce(
      (s, c) => s + c.orderAmount,
      0
    )
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    logger.error("[Campaigns] PATCH", error)
