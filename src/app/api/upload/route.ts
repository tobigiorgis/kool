  ...
  try {
    ...
    if (!ALLOWED_TYPES.includes(file.type)) {
      return NextResponse.json(
        { error: "Tipo de archivo no permitido. Solo PDF, imágenes o documentos Word." },
        { status: 400 }
      )
    }
    ...
    const token = env.BRIEFDOCS_READ_WRITE_TOKEN
    if (!token)
      return NextResponse.json({ error: "BRIEFDOCS_READ_WRITE_TOKEN no configurado" }, { status: 500 })
    ...
    return handleError("[upload] POST", error)
  }
}
