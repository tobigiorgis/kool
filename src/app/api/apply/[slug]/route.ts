  ...
  try {
    ...
    if (campaign.formStatus === "CLOSED") {
      return NextResponse.json(
        { error: "Esta campaña ya no está aceptando aplicaciones." },
        { status: 410 }
      )
    }
    if (campaign.formStatus === "PAUSED") {
      return NextResponse.json(
        { error: "Las aplicaciones están temporalmente pausadas." },
        { status: 503 }
      )
    }
    ...
    const fields = (campaign.fields ?? {}) as Record<
      string,
    ...
    if (existing) {
      return NextResponse.json(
        { error: "Ya enviaste tu aplicación a esta campaña." },
        { status: 409 }
      )
    }
    ...
    }).catch((err) => logger.error("[Apply] Confirmation email error", err))
