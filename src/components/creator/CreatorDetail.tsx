    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        <MetricTile
          ...
          label="Clicks"
          value={creator.totalClicks.toLocaleString()}
        />
        <MetricTile
      ...
      <div>
        ...
        <div className="space-y-3 mt-3">
          ...
          {creator.phone && (
            <InfoRow icon={<Phone size={13} />} label="Teléfono" value={creator.phone} />
          )}
      ...
      {creator.discountCode && (
        <div>
          ...
          <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-xl mt-3">
            <span className="text-sm font-mono font-medium text-gray-900 flex-1">
              {creator.discountCode}
            </span>
        ...
        >
          <div className="flex items-center justify-between mb-1.5">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-rose-500">{shortUrlLabel(link.slug)}</span>
            </div>
            <button
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-xs text-gray-500">
              <span className="font-medium text-gray-900">{formatCurrency(link.revenue)}</span>{" "}
              revenue
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-gray-50 rounded-xl p-3 text-center">
          ...
          <p className="text-base font-semibold text-amber-600">
            {formatCurrency(creator.pendingCommissions)}
          </p>
        </div>
        <div className="bg-gray-50 rounded-xl p-3 text-center">
...
function MetricTile({
  ...
  return (
    <div className={`rounded-xl p-4 ${accent ? "bg-brand-50" : "bg-gray-50"}`}>
      <div
        className={`flex items-center gap-1.5 mb-1 ${accent ? "text-brand-400" : "text-gray-400"}`}
      >
        {icon}
        <span
          className={`text-[11px] uppercase tracking-wider ${accent ? "text-brand-500" : "text-gray-400"}`}
        >
          {label}
        </span>
      </div>
      <p className={`text-xl font-semibold ${accent ? "text-brand-700" : "text-gray-900"}`}>
        {value}
      </p>
    </div>
  )
